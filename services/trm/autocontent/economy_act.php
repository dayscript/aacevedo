<?php
  ...
  include ( "app/class/HtmlParser.class.php" );
  include ( "app/class/DataBase.class.php" );
  include ( "app/class/DataKeeper.class.php" );

  $dKeeper = new DataKeeper( );
  $dKeeper->setDebug( 0 );
  $dKeeper->setShowErrors( false );

  $dKeeper->setConnection( "localhost", "forge", "cOgjZoIGk9eK3e4LFIfM", "services" );
  $dKeeper->setMainTable( "economy_test" );
  $dKeeper->setHistoryTable( "economy_history_test" );
...
/*  $dKeeper->setURL( "http://portal.banrep.gov.co/j2ee/encuesta/jsp/trm_del_dia.jsp", array( '<b', '</b', '\(', '\)' ) );
  ...
  */
    ...
    $str = str_replace(",", ".", $str);
    $dKeeper->saveData( 'trm', 'TRM', 'COP',$str, date("Y-m-d"), 0,'CO');
  ...
  $dKeeper->addData( "Libor 1 A?o", "libor1ano", "%", "Libor 1 A?o", "gb" );
  ...
  $dKeeper->addData( "DTF 90 D?as (E.A.)", "dtf90ea", "%", "DTF 90 D?as \(E.A.\)", "co" );
  $dKeeper->addData( "DTF 90 D?as (T.A.)", "dtf90ta", "%", "DTF 90 D?as \(T.A.\)", "co" );
  $dKeeper->addData( "DTF 180 D?as (E.A.)", "dtf180ea", "%", "DTF 180 D?as \(E.A.\)", "co" );
  $dKeeper->addData( "DTF 180 D?as (T.A.)", "dtf180ta", "%", "DTF 180 D?as \(T.A.\)", "co" );
  $dKeeper->addData( "DTF 360 D?as (E.A.)", "dtf360ea", "%", "DTF 360 D?as \(E.A.\)", "co" );
  $dKeeper->addData( "DTF 360 D?as (T.A.)", "dtf360ta", "%", "DTF 360 D?as \(T.A.\)", "co" );
  ...
  */
  ...
  $dKeeper->setURL( "http://www.upme.gov.co/GeneradorConsultas/Consulta_Indicador.aspx?Ind=39" );
  $dKeeper->addData( "Gasolina (Bogot?)", "gasolina_bta", "COP", date( "Y-m-d", $date ), "co" );
  ...
  $dKeeper->addData( "ACPM (Bogot?)", "acpm_bta", "COP", date( "Y-m-d", $date ), "co" );
  $dKeeper->parseData( '</table', array( "value" => 'str_replace( ",", ".", str_replace( ".", "", $data[1] ) )', "date" => '$data[0]' ) );
  ...
  //$dKeeper->addData( "Petr?leo (WTI)", "wti", "USD", 'Petr?leo WTI', "co" );
  $dKeeper->addData( "Caf?", "cafe", "USD", 'CAF?', "co" );
  //$dKeeper->addData( "Devaluaci?n Diaria", "devaluaciondia", "%", 'Devaluaci?n diaria', "co" );
  $dKeeper->addData( "Interbancaria E.A.", "interbancariaea", "%", 'Tasa Interbancaria', "co" );
  //$dKeeper->addData( "Libor (6 meses)", "libor6mes", "%", 'Libor \\(6 meses\\)', "gb" );
  //$dKeeper->addData( "Prime Rate (180 d?as)", "prime180dia", "%", 'Prime Rate \\(180 d?as\\)', "co" );
  ...
  //$dKeeper->addData( "Devaluaci?n Anual", "devaluacionano", "%", 'Devaluaci?n anual', "co" );
  ...
  try {
    ...
    $use = array(
      "bolivar" => array( "COP", "ve", "bolivar", "Bol?var" ),
